          ...
          {data.logo && (
            <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20">
              <img
                src={data.logo}
                alt="Logo"
                className="max-h-14 w-auto object-contain drop-shadow-2xl bg-white/30 backdrop-blur-lg px-2 pt-1 pb-3 rounded-2xl border border-white/40 -translate-y-0.5"
                crossOrigin="anonymous"
              />
            </div>
          )}
              ...
              <div key={section.id} className="space-y-5">
                ...
                <ul className="space-y-4 pr-5">
                  {section.items.map((item, idx) => (
                    <li
                      key={idx}
                      className="flex gap-4 items-start text-gray-800"
                    >
                      <span
